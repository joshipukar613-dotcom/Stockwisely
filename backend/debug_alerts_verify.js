...
async function run() {
    ...
    try {
        console.log('Running Low Stock Query (FIXED)...');
        // EDITED QUERY with movement_date
        const res = await client.query(`
      WITH latest_stock AS (
        SELECT DISTINCT ON (product_code)
          product_code, description, balance_qty, movement_date as created_at
        FROM stock_movements
        ORDER BY product_code, movement_date DESC NULLS LAST
      )
      SELECT 
        ls.product_code, 
        ls.description, 
